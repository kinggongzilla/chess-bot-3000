...
def get_datasets(
    ...
    if isinstance(hf_dataset_or_datasets, dict):
        ...
        for split in splits:
            # Check if it's a local path to a saved dataset
            if isinstance(hf_dataset_or_datasets, str) and Path(hf_dataset_or_datasets).exists():
                # Check if it's a dataset saved with save_to_disk (has dataset_info.json)
                if (Path(hf_dataset_or_datasets) / "dataset_info.json").exists():
                    raw_datasets[split] = load_from_disk(hf_dataset_or_datasets)
                    # If a specific split is requested, select it
                    if split and split in raw_datasets[split]:
                        raw_datasets[split] = raw_datasets[split][split]
                else:
                    # It's a local path but not a saved dataset, load normally
                ...
                # It's a HuggingFace dataset identifier, load normally
                raw_datasets[split] = load_dataset(
                    hf_dataset_or_datasets,
                    hf_dataset_config_name,
                    split=split,
